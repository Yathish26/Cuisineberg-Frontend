...
import { Pencil, Briefcase, Home, MapPin, ShoppingBag, Heart, CreditCard, Settings, Plus, XCircle, CheckCircle } from 'lucide-react'; // Added CheckCircle
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 401) {
            localStorage.removeItem('token'); // Fixed localStorage access
            navigate('/login');
          } else {
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center">
          ...
          {!isEditing ? (
            <button
              ...
              className="flex items-center px-4 py-2 bg-white text-blue-900 rounded-full shadow-lg hover:bg-gray-100 transition duration-300 ease-in-out text-sm font-semibold"
            >
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
              <button
                ...
                className="flex items-center justify-center px-4 py-2 bg-gray-300 text-gray-800 rounded-full shadow-lg hover:bg-gray-400 transition duration-300 ease-in-out text-sm font-semibold"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                className="flex items-center justify-center px-4 py-2 bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 transition duration-300 ease-in-out text-sm font-semibold"
              >
                Save Changes
              </button>
            </div>
